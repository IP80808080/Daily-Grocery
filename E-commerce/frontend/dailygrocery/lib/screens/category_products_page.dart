    ...
    if (hasMoreData) {
      ...
      try {
        List<Product> newProducts = await productService.fetchProducts(
            'category-product', widget.categoryName, limit, offSet);

        if (newProducts.isNotEmpty) {
          List<Product> remainingProducts = newProducts
              .where((product) => !_products.contains(product))
              .toList();

          if (remainingProducts.isNotEmpty) {
            setState(() {
              _products.addAll(remainingProducts);
            });
          } else {
            setState(() {
              hasMoreData = false;
      ...
      appBar: AppBar(
        ...
        title: GestureDetector(
          ...
          child: _isSearching
              ...
              : Text(
                  'Products in ${utf8.decode(widget.categoryName.runes.toList())}'),
        ),
